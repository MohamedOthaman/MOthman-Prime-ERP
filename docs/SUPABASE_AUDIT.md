...
> **CI confirmed this is pre-existing and not caused by this PR.** The Supabase Preview branch rebuilds the schema from migration files in timestamp order and **aborts at `20260325150000_phase_2_5_schema_alignment.sql` (statement 6: `CREATE INDEX ... ON public.customers (salesman_id)`)** — because `customers` is not created until `20260328123000_phase_5_master_reference_data_import.sql`, three days *later* in the ordering. The build never reaches this PR's migration (Jun 04). **No change to this PR can make the Preview check pass**; it requires repairing the migration history (reorder/guard the many out-of-order references that only work because the live DB pre-existed the migrations) — a separate effort. The `to_regclass()` guards in this PR's migration are retained as defensive/idempotent hygiene for the manual live apply, **not** as a preview-CI fix (they cannot be, since the build fails ~30 migrations earlier).
