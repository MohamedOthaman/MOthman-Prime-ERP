...
**`supabase/migrations/20260604120000_p1_fk_covering_indexes.sql`** — 39 covering indexes for the 39 advisor-flagged unindexed FKs. Every column verified against live `pg_constraint`, every target cross-checked against live `pg_indexes` (no duplicates). All `CREATE INDEX IF NOT EXISTS` (idempotent, additive, reversible).
