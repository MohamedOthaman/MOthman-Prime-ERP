...
export class PdfExtractor implements ExtractorProvider {
  ...
  async extract(file: File, hints?: ExtractionHints, onProgress?: (msg: string) => void): Promise<ExtractionResult> {
    ...
    if (pdfType === "invoices") rows = (result.invoices ?? []) as unknown as Record<string, unknown>[];
    else if (pdfType === "sku") rows = (result.products ?? []) as unknown as Record<string, unknown>[];
    else rows = (result.items ?? []) as unknown as Record<string, unknown>[];
