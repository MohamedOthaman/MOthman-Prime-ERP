...
export function TopBar() {
  ...
  const { t } = useLang();
  const { tier, canAccessSettings } = usePermissions();
      ...
      <div className="max-w-7xl mx-auto px-4 h-11 flex items-center gap-2">
        {isDesktop && (
          <button
            ...
            title={state === "collapsed" ? t("expandSidebar") : t("collapseSidebar")}
            aria-label={state === "collapsed" ? t("expandSidebar") : t("collapseSidebar")}
        ...
        {/* Settings button */}
        <button
          onClick={() => navigate(canAccessSettings ? "/admin/settings" : "/profile")}
        ...
        <button
          ...
          title={t("logout")}
          className="flex items-center justify-center gap-1 bg-secondary text-secondary-foreground px-2 py-1 rounded-md text-xs font-semibold border border-border hover:bg-destructive/15 hover:text-destructive hover:border-destructive/30 transition-colors shrink-0 sm:w-28 whitespace-nowrap"
        >
