...
export default function FinanceDashboard() {
  ...
  const { invoices, returns, loading } = useFinanceData();
  ...
  const kpis: KpiItem[] = [
    ...
    },
    {
      ...
      sub: "Total cancelled",
      icon: XCircle,
      color: invoices?.cancelled ? "text-red-500" : "text-muted-foreground",
      bg: invoices?.cancelled ? "bg-red-500/10" : "bg-muted/20",
      border: invoices?.cancelled ? "border-red-500/20" : "border-border",
      loading,
    },
  ];
  ...
  return (
    <DashboardShell
      ...
      subtitle={`${roleLabel} · Invoice & Returns Management`}
      accent="blue"
      headerAction={
        <button
