...
export default function FridgeStoragePage() {
  ...
  // ─── Storage config (inside component for i18n) ──────────────────────────────
  const STORAGE_CONFIG = useMemo(() => ({
    Frozen:  { label: t("storageFrozen", "Frozen"),   icon: ThermometerSnowflake, color: "text-cyan-400",   bg: "bg-cyan-500/8",    border: "border-cyan-500/20",   headerBg: "bg-cyan-500/10"   },
    Chilled: { label: t("storageChilled", "Chilled"), icon: Wind,                 color: "text-blue-400",   bg: "bg-blue-500/8",    border: "border-blue-500/20",   headerBg: "bg-blue-500/10"   },
    Dry:     { label: t("storageDry", "Dry"),         icon: Flame,                color: "text-amber-400",  bg: "bg-amber-500/8",   border: "border-amber-500/20",  headerBg: "bg-amber-500/10"  },
  }), [t]);
  ...
  const grouped = useMemo(() => {
    ...
    for (const tp of [...STORAGE_ORDER, "Other"]) map.set(tp, []);
  ...
  const summary = useMemo(() => {
    ...
    for (const b of batches) {
      const tp = b.storage_type;
      if (!totals[tp]) totals[tp] = { count: 0, expired: 0, near: 0 };
      totals[tp].count++;
      if ((b.days_to_expiry ?? 1) < 0) totals[tp].expired++;
      else if ((b.days_to_expiry ?? 999) <= 30) totals[tp].near++;
    }
    return totals;
  }, [batches]);
  ...
  const filterTabs = useMemo(() => [
    ...
    ...STORAGE_ORDER.map((tp) => ({
      key: tp,
      label: (STORAGE_CONFIG as any)[tp]?.label ?? tp,
    })),
  ], [t, STORAGE_CONFIG]);
        ...
        <div className="max-w-5xl mx-auto px-4 py-3.5 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-[15px] font-bold tracking-tight text-foreground leading-tight">
              {t("pageTitleColdStorage", "Cold Storage")}
            </h1>
            <p className="text-[11px] text-muted-foreground leading-tight">
              {loading
                ? t("loading", "Loading…")
                : `${filtered.length} ${t("batches", "batches")} · ${t("fefoOrder", "FEFO order")}`}
            </p>
          </div>
          <button
            onClick={load}
        ...
        <div className="max-w-5xl mx-auto px-4 pb-3 flex gap-2 flex-wrap">
          ...
          <div className="relative flex-1 min-w-[160px]">
            ...
            <input
              ...
              placeholder={t("searchProductBatchLocation", "Search product, batch, location…")}
      ...
      <main className="max-w-5xl mx-auto px-4 py-4 space-y-5">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <button onClick={load} className="text-xs text-primary hover:underline">{t("retry", "Retry")}</button>
          </div>
        ) : grouped.length === 0 ? (
          <EmptyState
            icon={Package}
            message={t("noStockFound", "No stock found")}
            sub={t("noStockFoundSub", "Stock will appear here once received and posted to inventory")}
          />
        ) : (
          grouped.map(([storageType, rows]) => {
            const cfg = (STORAGE_CONFIG as any)[storageType] ?? STORAGE_CONFIG.Dry;
            const Icon = cfg.icon;
                ...
                <div className={`flex items-center gap-3 px-4 py-3 ${cfg.headerBg}`}>
                  ...
                  {stats?.near > 0 && stats?.expired === 0 && (
                    <span className="ml-auto text-[10px] font-semibold text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded px-1.5 py-0.5">
                      {stats.near} {t("nearExpiry", "near expiry")}
                    </span>
                  )}
                </div>
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-border/60 text-muted-foreground">
                        <th className="px-4 py-2 text-left font-medium">{t("colProduct", "Product")}</th>
                        <th className="px-3 py-2 text-left font-medium">{t("colBatch", "Batch")}</th>
                        <th className="px-3 py-2 text-left font-medium">{t("colLocation", "Location")}</th>
                        <th className="px-3 py-2 text-right font-medium">{t("colQty", "Qty")}</th>
                        <th className="px-3 py-2 text-center font-medium">{t("colExpiry", "Expiry")}</th>
                        <th className="px-3 py-2 text-center font-medium">{t("colDays", "Days")}</th>
                    ...
                    <tbody>
                      {rows.map((row) => {
                        ...
                        const isExp = (row.days_to_expiry ?? 1) < 0;
                        return (
                          <tr
                            key={row.id}
                            className={`border-t border-border/40 hover:bg-muted/20 transition-colors cursor-pointer ${isExp ? "bg-red-500/3" : ""}`}
