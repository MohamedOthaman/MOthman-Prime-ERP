...
function expiryLabel(days: number | null, t: (key: string, fallback: string) => string): string {
  if (days === null) return "—";
  if (days < 0)   return `${t("expired", "Expired")} (${Math.abs(days)}d ago)`;
...
export default function ProductPerformanceReport() {
  ...
  const kpis: KpiItem[] = [
    { label: t("activeSkus", "Active SKUs"), value: loading ? "—" : rows.length, icon: Package, color: "text-emerald-400", bg: "bg-emerald-500/8", border: "border-emerald-500/20", loading },
    { label: t("outOfStock", "Out of Stock"), value: loading ? "—" : zeroStock, icon: TrendingDown, color: "text-red-400", bg: "bg-red-500/8", border: "border-red-500/20", loading, trend: zeroStock > 0 ? "down" : "neutral" },
    ...
    { label: t("expiredInStock", "Expired (in stock)"), value: loading ? "—" : expired, icon: AlertTriangle, color: "text-red-400", bg: "bg-red-500/8", border: "border-red-500/20", loading, trend: expired > 0 ? "down" : "neutral" },
  ];
  ...
  const tableHeaders = useMemo(() => [
    ...
    t("batches", "Batches"),
    t("nearestExpiry", "Nearest Expiry"),
    t("moved30d", "Moved (30d)"),
  ], [t]);
  ...
  return (
    <DashboardShell
      ...
      subtitle={t("productPerformanceSubtitle", "Stock levels, expiry status, and 30-day sales velocity")}
      accent="emerald"
      headerAction={
        <button
    ...
    >
      ...
      {/* Filters */}
      <SectionCard title={t("searchAndFilter", "Search & Filter")} icon={Search} iconClass="text-muted-foreground">
        <div className="flex flex-wrap gap-3">
          <div className="flex items-center gap-2 flex-1 min-w-[200px] rounded-lg border border-border bg-background px-3 py-1.5">
            ...
            <input
              type="text"
              placeholder={t("searchNameCode", "Search name, code...")}
              value={search}
          ...
          <select
            ...
            className="rounded-lg border border-border bg-background px-3 py-1.5 text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-primary"
          >
            <option value="qty">{t("sortStockHighLow", "Sort: Stock High–Low")}</option>
            <option value="expiry">{t("sortNearestExpiry", "Sort: Nearest Expiry")}</option>
            <option value="velocity">{t("sortMostMoved30d", "Sort: Most Moved (30d)")}</option>
          </select>
        </div>
      </SectionCard>

      {/* Table */}
      <SectionCard title={`${t("products", "Products")} (${filtered.length})`} icon={Package} iconClass="text-emerald-400">
        {error ? (
                ...
                {filtered.map(r => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className={`py-2.5 px-2 text-xs ${expiryClass(days)}`}>{expiryLabel(days, t)}</td>
