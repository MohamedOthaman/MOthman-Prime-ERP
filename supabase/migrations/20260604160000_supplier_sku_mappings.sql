-- ════════════════════════════════════════════════════════════════
-- Supplier SKU Mappings + Duplicate Invoice Detection
-- Applied: 2026-05-29
-- ════════════════════════════════════════════════════════════════

-- ────────────────────────────────────────────────────────────────
-- 1. Supplier SKU mappings
--    Maps supplier product codes / names → internal product_id.
--    Used in GRN matching (Priority 3, after barcode + item code).
-- ────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.supplier_sku_mappings (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    supplier_id     UUID REFERENCES public.suppliers(id)  ON DELETE CASCADE,
    external_code   TEXT,       -- supplier's item code printed on PO/invoice
    external_name   TEXT NOT NULL,  -- supplier's product name
...
-- ────────────────────────────────────────────────────────────────
-- 2. Duplicate invoice detection function
--    Checks for existing sales_headers with same invoice_no
--    (case-insensitive) OR same customer + date + total_amount.
--    Returns JSON with { is_duplicate, existing_id, reason }.
-- ────────────────────────────────────────────────────────────────
...
DECLARE
    ...
    v_reason        TEXT;
BEGIN
    -- Check 1: exact invoice number match (case-insensitive)
    ...
    -- Check 2: same customer + date + amount (fuzzy duplicate)
...
-- ────────────────────────────────────────────────────────────────
-- 3. upsert_supplier_sku_mapping helper
--    Called from extraction pipeline when a supplier product
--    is successfully matched to an internal product.
-- ────────────────────────────────────────────────────────────────
